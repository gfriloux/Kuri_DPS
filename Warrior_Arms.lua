...
table.insert(WARRIOR_DPS_ROTATION,
{
  ...
  condition = function()
    if not Zorlen_HasTalent(LOCALIZATION_ZORLEN.MortalStrike) then
      return false
    end

    if        UnitMana("player") < 30
       or not Zorlen_checkCooldownByName(LOCALIZATION_ZORLEN.MortalStrike) then
      return false
    end
    return true
  end
