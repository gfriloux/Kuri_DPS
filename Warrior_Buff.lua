...
table.insert(WARRIOR_BUFF_ROTATION,
{
  ...
  execution = function()
    Zorlen_useEquippedItemByName("Kiss of the Spider")
  end,
  -- If Earthstrike is equiped, we use it when it is UP.
  condition = function()
    if   not Zorlen_isItemByNameEquipped("Kiss of the Spider")
      or     Zorlen_notInCombat()
    then
      return false
    end
...
table.insert(WARRIOR_BUFF_ROTATION,
{
  ...
  condition = function()
    ...
    if not Zorlen_HasTalent(LOCALIZATION_ZORLEN.DeathWish) then
      return false
    end
    ...
    if    not Zorlen_inCombat()
       ...
       or     WE_WANT_AGGRO           ==  1
       or not Zorlen_isCurrentRaceOrc ==  1
       or not Zorlen_checkCooldownByName("Death Wish")
    then
      return false
    end
    return true
...
table.insert(WARRIOR_BUFF_ROTATION,
{
  name      = LOCALIZATION_ZORLEN.Berserking,
  execution = function()
    Zorlen_castSpellByName(LOCALIZATION_ZORLEN.Berserking)
  end,
  -- We want to maximize our rage to be able to cast instant skills
  condition = function()
    local percent = (UnitHealth("player") / UnitHealthMax("player")) * 100
    ...
    if    not Zorlen_inCombat()
       or     percent                  >= 50
       or not Zorlen_isCurrentRaceTroll ==  1
       or not Zorlen_checkCooldownByName(LOCALIZATION_ZORLEN.Berserking)
    then
      return false
    end
    return true
